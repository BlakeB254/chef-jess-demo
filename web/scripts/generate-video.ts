...
async function generateHeroVideo() {
  ...
  console.log('Initializing video generation with Veo 2 (Image-to-Video)...');

  // The correct chef image based on user feedback (Caesar Salad card image)
  ...
  const mimeType = 'image/jpeg'; // Assuming jpg based on extension

  const prompt = "Cinematic sequence of this professional chef in a beautiful, sunlit luxury kitchen. She is smiling while finishing plating a gourmet dish. The camera pulls back to reveal a table full of beautiful, colorful meals. Photorealistic, 4k, elegant lighting, high end culinary art, stable video, no distortion.";
  ...
  try {
    ...
    const outputPath = path.join(outputDir, 'hero-video.mp4');
